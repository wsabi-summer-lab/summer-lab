...
nba_posterior <- nba_variance_posterior %>%
  ...
  mutate(
    ...
    mu_bij = NA_real_,
    game_num = row_number()
    w = 1 / (sigma2_hat + game_num * tau2_hat)
  
  ) %>% 
  group_split()
  
for (i in seq_along(nba_posterior)) {
  player_df <- nba_posterior[[i]]  # use [[i]] not [i]
  n_games <- nrow(player_df)
  
  mu_vec <- numeric(n_games)
  mu_vec[1] <- mu_hat  # initialize with global prior
  
  for (j in 2:n_games) {
    games <- j - 1
    X_j <- player_df$X[j]
    mu_vec[j] <- mu + (.001/(.001+sigma2_hat)) * (ppp - mu_vec[j-1])
  }
