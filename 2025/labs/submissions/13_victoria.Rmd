...
putting <- putting %>% 
  mutate(
    C = p1*(1-p1),
    Xi_tilde = X/(sqrt(C/N)), 
    Xi_tilde_bar = sum(Xi_tilde)/length(putting),
...
putting <- putting %>% 
  mutate(
    ...
    new_Xi_tilde_bar = sum(new_Xi_tilde)/length(putting),
    new_Sx2 = sum(new_Xi_tilde - Xi_doubletilde_bar)^2,
    theta_em2 = new_Xi_tilde_bar +  (1-((length(putting)-1)/new_Sx2)) *(new_Xi_tilde - new_Xi_tilde_bar),
...
putting <- putting %>% 
  mutate(
    Sx2_5 = 1/(length(putting)-1) * sum(X-p),
    tau2 = Sx2_5 - C*mean(1/N),
    p5 = p + tau2/(tau2+(C/N))*(X-p)
  )
...
putting <- putting %>% 
  mutate(
    ...
    Xi_tilde_6_bar = sum(Xi_tilde_6)/length(putting),
    Sx2_6 = 1/(length(putting)-1)*sum(new_Xi_tilde- new_Xi_tilde_bar)^2,
    new_tau2 = new_Sx2 - sum(nu_i2)/length(putting),
...
tibble(
  ...
  mse = c(
    mse(putting_cleaned$p1, putting_test$X),
    mse(putting_cleaned$X, putting_test$X),
    mse(putting_cleaned$p3, putting_test$X),
    mse(putting_cleaned$p4, putting_test$X),
    mse(putting_cleaned$p5, putting_test$X),
    mse(putting_cleaned$p6, putting_test$X)
  )
)
